...
async function _seed(): Promise<void> {
  try {
    ...
    const admin = await kv.get<User>(`user:iimcadmin2025`);
    if (admin?.isAdmin) return; // already seeded

    // Seed admin
    await kv.set(`user:iimcadmin2025`, SEED_ADMIN);
    await kv.sadd('user_list', 'iimcadmin2025');
