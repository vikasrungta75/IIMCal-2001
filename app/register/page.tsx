...
export default function RegisterPage() {
  ...
  useEffect(() => {
    if (status === 'authenticated') { router.replace('/auth-redirect'); }
  }, [status]);
  ...
  return (
    <div className="min-h-screen flex" style={{ background: 'linear-gradient(160deg,#003366 0%,#001a33 55%,#0a1628 100%)' }}>
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden flex-col justify-end p-12">
        <img src="/images/campus.jpg" alt="IIM Calcutta" className="absolute inset-0 w-full h-full object-cover opacity-30" />
        <div className="absolute inset-0" style={{ background: 'linear-gradient(to top, rgba(0,26,51,0.95) 30%, rgba(0,51,102,0.3) 100%)' }} />
        <div className="relative z-10">
          <div className="badge-gold inline-block mb-4">Silver Jubilee 2026</div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 py-20">
        <div className="w-full max-w-md">
          ...
          <div className="rounded-2xl p-8" style={{ background: 'rgba(255,255,255,0.97)' }}>
            {/* How it works */}
            <p className="text-sm font-semibold mb-4" style={{ color: '#003366' }}>How registration works</p>
            <div className="space-y-3 mb-6">
              {[
                { n: '1', t: 'Sign in with Google or Microsoft below' },
                { n: '2', t: 'Fill in your Batch 2001 IIMC profile' },
                { n: '3', t: 'Admin reviews and approves your registration' },
                { n: '4', t: 'You receive an email — then log in to access the portal' },
              ].map(({ n, t }) => (
                <div key={n} className="flex items-center gap-3">
                  <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold text-white flex-shrink-0"
                    style={{ background: '#003366' }}>{n}</div>
            ...
            <p className="text-center mt-5 text-sm text-gray-500">
              Already registered?{' '}
              <Link href="/login" className="font-semibold hover:underline" style={{ color: '#003366' }}>Sign in →</Link>
            </p>
          </div>
